	...
	, planes_(faces_.size())
{
	...
	for (uint8_t i = 0; i < 3; i++)//вычислние центтра т€жести
	{
	...
	// plane equation
	for (uint32_t j = 0; j < faces_.size(); j++)//вычисление коофицентов плоскостей
	{
		auto &f = faces_[j];
		auto &p = planes_[j];
...
void Facets::Draw(Graphics &g)
{
	...
	for (uint32_t j = 0; j < faces_.size(); j++)
	{
		...
		if (p[0] * Camera::x() + p[1] * Camera::y() + p[2] * Camera::z() < 0)//проверка лицевой грани
		{
